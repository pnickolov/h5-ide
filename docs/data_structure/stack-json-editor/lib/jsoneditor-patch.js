  ...
  if (domValue) {
	...
	if (res_list.indexOf('|'+field+'|') != -1)
	{
		...
		for (var key in res)
		{
			...
			{
				for (var api_key in res[key] )
				{
					var _url=res[key][api_key],
						_fieldname=api_key;
					link_ary.push(' &lt;<a href="http://docs.aws.amazon.com/'+_url+'.html" target="_blank">'+_fieldname+'</a>&gt;');
				}
			}
		}
