var EventEmitter, Helper, Q, StreamFuncs, cached, changeHandler, checkWatchHealthy, chokidar, coffee, coffeelint, coffeelintOptions, compileCoffeeOnlyRegex, compileDev, confCompile, es, fs, globwatcher, gulp, gulpif, gutil, handlebars, jshint, langsrc, lintReporter, path, tinylr, util, watch;
...
Helper = {
  ...
  createWatcher: function() {
    ...
    if (GLOBAL.gulpConfig.pollingWatch) {
      ...
      gulp.watch(["./src/**/*.coffee", "./src/**/*.html", "./src/**/*.partials", "./src/assets/**/*", "!src/include/*.html"], function(event) {
        var type;
        if (event.type === "added") {
          type = "add";
...
compileDev = function() {
  var compileStream, d, p;
  p = ["src/**/*.coffee", "src/**/*.partials", "src/**/*.html", "!src/nls/*.coffee", "!src/*.html", "!src/include/*.html"];
  d = Q.defer();
  StreamFuncs.createStreamObject();
  ...
  compileStream.once("end", function() {
    return Q.all([Helper.compilei18n(), Helper.compileCompass()]).then(function() {
      return d.resolve();
    });
  });
  return d.promise;
