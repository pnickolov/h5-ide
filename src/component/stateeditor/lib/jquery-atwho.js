  ...
  (function(factory) {
    ...
    App = (function() {
      ...
      App.prototype.listen = function() {
        ...
        }).on('focus.atwho', function(e) {
          var c;
          if (c = _this.controller()) {
            if (!c.get_opt('at') && !_this.$inputor.val()) {
              return _this.on_keyup(e);
            }
          }
        });
      ...
      App.prototype.dispatch = function() {
        ...
        return $.map(this.controllers, function(c) {
          var content, data, haveComplete;
          data = c.get_opt('data');
          content = c.content();
          haveComplete = false;
          _.each(data, function(dataObj) {
            if (dataObj.name === content && content) {
              haveComplete = true;
            }
            return null;
          });
    ...
    Controller = (function() {
      ...
      Controller.prototype.rect = function() {
        var c, leftPos, scale_bottom, topPos;
        c = this.$inputor.caret('offset', this.pos);
        ...
        if (this.at) {
          ...
          leftPos = this.$inputor.offset().left;
          topPos = this.$inputor.offset().top;
          c = {
            top: topPos,
            height: this.$inputor.height() + 6,
            bottom: topPos
          };
        }
        return {
      ...
      Controller.prototype.insert = function(content, $li) {
        var $inputor, $insert_node, class_name, content_node, insert_node, pos, range, sel, source, start_str, suffixStr, text;
        ...
        if ($inputor.is('textarea, input')) {
          ...
          suffixStr = "";
          if (this.get_opt('at')) {
            suffixStr = "" + (source.slice(this.query['end_pos'] || 0));
          }
    ...
    View = (function() {
      ...
      View.prototype.choose = function() {
        var $li, content, onSelectedCallback, value;
        $li = this.$el.find(".cur");
        content = this.context.insert_content_for($li);
        this.context.insert(this.context.callbacks("before_insert").call(this.context, content, $li), $li);
        this.context.trigger("inserted", [$li]);
        this.hide();
        onSelectedCallback = this.context.get_opt('onSelected');
        if (onSelectedCallback) {
          value = $li.attr('data-value');
          return onSelectedCallback.apply(this.context.$inputor[0], [value]);
        }
      };
