	...
	<li class="state-item view {{#ifCond cmd_value '#'}}comment{{/ifCond}}" data-command="{{cmd_value}}" data-id="{{id}}">
		<div class="state-toolbar">
			<div class="state-action-wrap">
				<div class="state-remove icon-delete tooltip" data-tooltip="{{i18n 'STATE_TIP_DELETE_STATE'}}"></div>
	...
	<div id="state-editor-wrap">
		<div id="state-editor-body">
			<div id="state-editor" spellcheck="false" class="font-mono">
				{{#unless supported_platform}}<div class="state-warning">{{ i18n 'STATE_UNKNOWN_DISTRO_LBL' }}</div>{{/unless}}
				...
				<div class="state-have-state-container">
					<div class="state-editor-toolbar clearfix font-normal">
						...
						<div class="state-operate">
							<a id="state-toolbar-add" href="javascript:void(0)">{{i18n 'STATE_TOOL_ADD_STATE'}}</a>
...
<!-- state-template-log-detail-modal -->
<div id="modal-state-log-detail" style="width: 900px;">
	<div class="modal-header"><h3>{{i18n 'STATE_LOG_DETAIL_MOD_TIT'}}</h3><i class="modal-close">&times;</i> </div>
	<div class="modal-body">
		<textarea class="state-log-detail-content" readonly="readonly">{{content}}</textarea>
	</div>
	<div class="modal-footer">
		<button class="btn modal-close btn-silver">{{i18n 'STATE_LOG_DETAIL_MOD_CLOSE_BTN'}}</button>
	</div>
</div>
