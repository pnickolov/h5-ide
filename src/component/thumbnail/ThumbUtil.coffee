...
define ['UI.canvg', 'UI.download'], ()->
  ...
  thumbBeforeRender = (ctx) ->
    ...
    ctx.canvas.width  = pattern.width  = 220
    ctx.canvas.height = pattern.height = 145

    patternctx = pattern.getContext("2d")
    patternctx.fillStyle = patternctx.createPattern(GridBackground, "repeat")
  ...
  exportPNG = ( $svg_canvas_element, data ) ->
    ...
    svg_canvas_element = $svg_canvas_element[0]
    size               = svg_canvas_element.getBBox()
    clone              = svg_canvas_element.cloneNode(true)
    beforeRender       = null
    ...
    $wrap.append(clone).attr "class", $("#canvas_container").attr("class")
