...
require [ 'MC', 'jquery', 'test_util', 'session_service', 'securitygroup_service'], ( MC, $, test_util, session_service, securitygroup_service ) ->
    ...
    #-----------------------------------------------
    test_DescribeDBSecurityGroups = () ->
        asyncTest "/aws/rds securitygroup.DescribeDBSecurityGroups()", () ->
            sg_name = null
            marker = null
            max_records = null

            securitygroup_service.DescribeDBSecurityGroups {sender:this}, username, session_id, region_name, sg_name, marker, max_records, ( aws_result ) ->
