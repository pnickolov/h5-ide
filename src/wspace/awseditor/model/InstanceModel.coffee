...
define [
  ...
  Model = ComplexResModel.extend {
    ...
    isMesosMaster: ( data ) ->
      states = data.state
      if states and states[states.length-1] and states[states.length-1].module is 'linux.mesos.master'
        return true
      return false

    isMesosSlave: ( data ) ->
      states = data.state
      if states and states[states.length-1] and states[states.length-1].module is 'linux.mesos.slave'
        return true
      return false
