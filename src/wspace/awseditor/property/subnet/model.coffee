  ...
  SubnetModel = PropertyModel.extend {
    ...
    init : ( uid ) ->
      ...
      @set {
        ...
        tags       : subnet_component.tags()
      }

      if @isAppEdit

        subnet = CloudResources(Design.instance().credentialId(), constant.RESTYPE.SUBNET, Design.instance().region()).get(subnet_component.get('appId'))?.toJSON()
