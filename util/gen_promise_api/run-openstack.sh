...
function fn_generate_coffee() {
    ...
    _SERVICE_l=${SERVICE,,}    #tolower    -> eg: session

    # if [ ${__CUR_FILE} != "Service.py" ]
    # then
    #     return
    # fi
    ...
    echo "__TGT_DIR_SERVICE: ${__TGT_DIR_SERVICE} | grep /${_RESOURCE_l}"
    FOUND=`echo ${__TGT_DIR_SERVICE} | grep "/${_RESOURCE_l}" | wc -l`
    if [ ${FOUND} -eq 1 -a ${_RESOURCE_l} != "service" ]
    then
...
#===================================================================
# Scan python file for openstack
#===================================================================
function fn_scan_openstack() {
    ...
    mkdir -p ${TGT_BASE_DIR}/"service"/openstack/
    ...
    #OSUtil#########################
        ...
        echo "########################################################"
        echo "#Processing OpenStack - "${CUR_FILE}
        echo "########################################################"

        _RESOURCE=${SERVICE/Util/}
        TGT_DIR=${TGT_BASE_DIR}/"service"/openstack/${_RESOURCE,,}  #lower
        #create subdir in out.tmp
        #mkdir -p ${TGT_DIR}                             #create out.tmp/service/

        fn_generate_coffee "osutil" "${CUR_DIR}" "${CUR_FILE}" "${TGT_DIR}"
    ...
    #Except OSUtil###################
        ...
        #resource
            ...
            echo "########################################################"
            echo "#Processing OpenStack - "${SERVICE}" - "${CUR_FILE}
            echo "########################################################"
            ...
            _RESOURCE=${RESOURCE/Util/}                                          #remove util
            TGT_DIR=${TGT_BASE_DIR}/"service"/openstack/${SERVICE,,}    #lower

            #create subdir in out.tmp
            #mkdir -p ${TGT_DIR}                             #create out.tmp/service/

            fn_generate_coffee "openstack" "${CUR_DIR}/${SERVICE}" "${CUR_FILE}" "${TGT_DIR}"
        done
...
do
    ...
    do
        ...
        elif [ "${SRC_DIR[$i]}" == "OpenStack" ]
        then
            fn_scan_openstack "${CUR_SRC_DIR}" "${LINE}"
        fi
    done
done
echo "end"
...
then
    echo "merge cinder"
    rm -rf ${TGT_BASE_DIR}/service/openstack/cinder.tmp
    for LINE in `ls ${TGT_BASE_DIR}/service/openstack/cinder_*.js`
    do
        cat ${LINE} | grep "method:" >> ${TGT_BASE_DIR}/service/openstack/cinder_.tmp
    done
    #cat ${TGT_BASE_DIR}/service/openstack/cinder.tmp
    sed '/cinder_GetAbsoluteLimits/r out.tmp/service/openstack/cinder_.tmp' ${TGT_BASE_DIR}/service/openstack/cinder.js > ${TGT_BASE_DIR}/service/openstack/cinder2.js
    rm -rf ${TGT_BASE_DIR}/service/openstack/cinder_*.*
    mv ${TGT_BASE_DIR}/service/openstack/cinder2.js ${TGT_BASE_DIR}/service/openstack/cinder.js
fi

LINE_LST=""
if [ -f ${TGT_BASE_DIR}/service/openstack/neutron.js ]
then
    echo "merge neutron"
    rm -rf ${TGT_BASE_DIR}/service/openstack/neutron.tmp
    for LINE in `ls ${TGT_BASE_DIR}/service/openstack/neutron_*.js`
    do
        cat ${LINE} | grep "method:" >> ${TGT_BASE_DIR}/service/openstack/neutron_.tmp
    done
    #cat ${TGT_BASE_DIR}/service/openstack/neutron.tmp
    sed '/neutron_V2_Extension/r out.tmp/service/openstack/neutron_.tmp' ${TGT_BASE_DIR}/service/openstack/neutron.js > ${TGT_BASE_DIR}/service/openstack/neutron2.js
    rm -rf ${TGT_BASE_DIR}/service/openstack/neutron_*.*
    mv ${TGT_BASE_DIR}/service/openstack/neutron2.js ${TGT_BASE_DIR}/service/openstack/neutron.js
fi


LINE_LST=""
if [ -f ${TGT_BASE_DIR}/service/openstack/nova.js ]
then
    echo "merge nova"
    rm -rf ${TGT_BASE_DIR}/service/openstack/nova.tmp
    for LINE in `ls ${TGT_BASE_DIR}/service/openstack/nova_*.js`
    do
        echo "processing ${LINE}..." 
        cat ${LINE} | grep "method:" >> ${TGT_BASE_DIR}/service/openstack/nova_.tmp
    done
    #cat ${TGT_BASE_DIR}/service/openstack/nova.tmp
    sed '/nova_V2_Limits/r out.tmp/service/openstack/nova_.tmp' ${TGT_BASE_DIR}/service/openstack/nova.js > ${TGT_BASE_DIR}/service/openstack/nova2.js
    rm -rf ${TGT_BASE_DIR}/service/openstack/nova_*.*
